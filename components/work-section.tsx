...
import { primaryLink, work, workMore, workSection } from "@/lib/copy";
...
export const WorkSection = () => (
  ...
  >
    ...
    <div className="space-y-24 pt-20 md:space-y-32">
      {work.map((item) => {
        const primary = primaryLink(item.links);
        return (
            ...
            <div className="pl-7 md:grid md:grid-cols-12 md:gap-10 md:pl-10">
              <div className="md:col-span-5">
                ...
                {item.body.map((paragraph, index) => (
                  <p
                    key={`${item.slug}-body-${String(index)}`}
                    className="text-ink/75 mt-4 leading-relaxed"
                  >
                    {paragraph}
                  </p>
                ))}
                {/* The clickable live demo is the shortest trust path for a CTO, so
                  it gets the one primary button; everything else stays a quiet
